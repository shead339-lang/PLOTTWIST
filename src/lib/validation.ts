...
export const QuizAnswersSchema = z.object({
  ...
  role: z.string().optional().transform((v) => (v && v.trim() ? v : "hero")),
  alarm: defaultAnswer("snooze_17"),
  footsteps: defaultAnswer("turn_around"),
  windfall: defaultAnswer("spend_72"),
  ...
  crush_text: defaultAnswer("send_memes"),
  ending: defaultAnswer("heroic_confused"),
  // Fallbacks for any legacy/alternate question keys
  situation: defaultAnswer("everything_at_once"),
  severity: defaultAnswer("ridiculous"),
  emergency_strategy: defaultAnswer("coffee_emergency"),
